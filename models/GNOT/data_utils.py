...
from models.GNOT.mmgpt import GNOT
...
def get_model(args):
    ...
    if not args.time_dependent:
        ...
        trunk_size, theta_size, branch_sizes, output_size = args.dataset_config['input_dim'], args.dataset_config['theta_dim'], [3, args.time_step+3], args.time_step
        #output_size = args.dataset_config['output_dim'] if args.component in ['all', 'all-reduce'] else 1
...
class FNODataset(DGLDataset):
    ...
    def __normalize_y(self):
        if self.y_normalizer is None:
            ...
            # logger.info('point wise normalizer shape',self.y_normalizer.mean.shape, self.y_normalizer.std.shape)
        ...
        logger.info('Target features are normalized using pointwise unit normalizer')
        # logger.info('Target features are normalized using unit transformer')
    ...
    def __normalize_x(self):
        ...
        logger.info('Input features are normalized using unit transformer')
...
class MIODataset(DGLDataset):
    def __init__(self, 
        ...
        if not os.path.exists(self.cached_path):
            ...
            logger.info('Load dataset finished {}'.format(time.time()-time0))
            #### initialize dataset
            self.train = train
            ...
            if self.train:
                if train_num == 'all':   # use all to train
                    ...
                    if train_num > len(data_all):
                        logger.info('Warnings: there is no enough train data {} / {}'.format(train_num, len(data_all)))
                self.data_list = data_all[:self.train_num]
                logger.info('Training with {} samples'.format(self.train_num))
                ...
                if test_num == "all":
                    ...
                    if test_num > len(data_all):
                        logger.info('Warnings: there is no enough test data {} / {}'.format(test_num, len(data_all)))

                self.data_list = data_all[-self.test_num:]
                logger.info('Testing with {} samples'.format(self.test_num))
    ...
    def __sort_dataset__(self):
        ...
        logger.info('Dataset sorted by number of nodes')
        return


    def __normalize_y__(self):
        if self.y_normalizer is None:
            y_feats_all = torch.cat([g.ndata['y'] for g in self.graphs],dim=0)
            if self.normalize_y == 'unit':
                self.y_normalizer = UnitTransformer(y_feats_all)
                logger.info('Target features are normalized using unit transformer')
                logger.info(f"{self.y_normalizer.mean}, {self.y_normalizer.std}")


            elif self.normalize_y == 'minmax':
                self.y_normalizer = MinMaxTransformer(y_feats_all)
                logger.info('Target features are normalized using unit transformer')
                logger.info(f"{self.y_normalizer.max}, {self.y_normalizer.min}")
                ...
                logger.info('Target features are normalized using quantile transformer')
