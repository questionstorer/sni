...
from utils.data_utils import get_train_dolphinx_dataset
from models.GNOT.data_utils import get_model, get_loss_func, MIODataLoader
from models.GNOT.utils import get_seed, get_num_params
from models.GNOT.optimizer import Adam
...
def train(model, loss_func, metric_func,
    ...
    if resume_path is not None and os.path.exists(resume_path):
        ...
        best_val_metric = checkpoint.get('best_val_metric', np.inf)
        best_val_epoch = checkpoint.get('best_val_epoch', 0)
        if 'loss_train' in checkpoint:
            loss_train = checkpoint['loss_train'].tolist() if isinstance(checkpoint['loss_train'], np.ndarray) else checkpoint['loss_train']
        if 'loss_val' in checkpoint:
            loss_val = checkpoint['loss_val'].tolist() if isinstance(checkpoint['loss_val'], np.ndarray) else checkpoint['loss_val']
        if 'lr_history' in checkpoint:
            lr_history = checkpoint['lr_history'].tolist() if isinstance(checkpoint['lr_history'], np.ndarray) else checkpoint['lr_history']
        it = checkpoint.get('iteration', 0)
        logger.info(f"Resumed from epoch {start_epoch}, best val metric: {best_val_metric:.6f}")
    else:
    ...
    for epoch in range(start_epoch, end_epoch):
        ...
        if val_metric < best_val_metric:
            ...
            best_model_path = os.path.join('./data/chkpt/{}'.format(model_path))
            torch.save(checkpoint, best_model_path)
            logger.info(f"Saved best model checkpoint at epoch {epoch+1} with val metric: {best_val_metric:.6f}")
...
def train_batch(model, loss_func, data, optimizer, lr_scheduler, device, grad_clip=0.999):
    ...
    if torch.isnan(y_pred).any():
        logger.error("y_pred has nan")
    loss.backward()
    
    for param in model.parameters():
        if param.grad is not None:
            grad_tensor = param.grad  # This is a torch.Tensor containing the gradient
            if torch.isnan(grad_tensor).any():
                logger.error("Gradient has NaN values")
                logger.error(y_pred)
...
if __name__ == "__main__":
    ...
    train_dataset, test_dataset = get_train_dolphinx_dataset(args)
    ...
    logger.info(f"Saving model and result in ./../models/chkpt/{model_path}\n")
    ...
    if args.lr_method == 'cycle':
        logger.info('Using cycle learning rate schedule')
    ...
    logger.info('Training takes {} seconds.'.format(time.time() - time_start))

    # result['args'], result['config'] = args, config
    # checkpoint = {'args':args, 'model':model.state_dict(),'optimizer':optimizer.state_dict()}
    # torch.save(checkpoint, os.path.join('./data/chkpt/{}'.format(model_path)))
    model.eval()
